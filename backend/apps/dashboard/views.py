...
@login_required
def paciente_predict_ajax(request, paciente_id):
    try:
        p = Paciente.objects.get(id_paciente=paciente_id)
    except Paciente.DoesNotExist:
        return JsonResponse({"detail": "Paciente no encontrado."}, status=404)
    try:
        from apps.ml.engine import predict
        payload = {
            "edad": p.edad, "imc": p.imc,
            "glucosa": p.glucosa, "colesterol": p.colesterol,
