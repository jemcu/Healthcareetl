"""Entrenamiento y predicción del modelo clínico con ensemble (RF + XGB + GB),
feature engineering avanzado, selección de características y optimización
de hiperparámetros."""
...
def _normalize_diag(value):
    ...
    # Buscar coincidencias contra el mapa
    for key, canonical in DIAG_MAP.items():
        key_norm = _strip_accents(key).lower()
        if key_norm in s_clean:
            return canonical
    # Si no hay coincidencia, asignar a la clase canónica más cercana
    # o a "Sin diagnóstico" para evitar clases espurias
    return "Sin diagnóstico"
...
def _dataset(features: list) -> pd.DataFrame:
    ...
    # Eliminar clases con menos de 5 ejemplos (más estricto para mejor generalización)
...
def train() -> ModelMetrics:
    """Entrena el ensemble con feature engineering, selección de
    características, SMOTE y optimización de hiperparámetros."""
    ...
    # ── Escalar (necesario para algunos modelos, no daña a tree-based) ──
    ...
    k_features = min(50, n_features_orig, len(unique_classes) * 5)
    ...
    try:
        ...
        if n_neighbors >= 2:
            # SMOTETomek combina oversampling con undersampling para mejor balance
            smote = SMOTE(random_state=42, k_neighbors=n_neighbors)
            # Intentar SMOTE primero, si hay problemas con clases muy pequeñas
            try:
    ...
    min_samples_leaf = max(1, len(y_tr_bal) // (n_classes * 20))
    ...
    if use_smote:
        # Si usamos SMOTE, no necesitamos class_weight
        param_dist["rf__class_weight"] = [None]
    else:
        param_dist["rf__class_weight"] = ["balanced", "balanced_subsample", None]

    n_iter = min(40, len(param_dist) * 3)
    ...
    y_pred = best_ensemble.predict(X_te_sel)
    ...
    best_params["feature_selector"] = "mutual_info_classif"

    # Extraer feature importances del mejor modelo
    feature_importances = {}
    try:
        # Para VotingClassifier, obtener importancia de cada sub-modelo
            ...
            if hasattr(estimator, "feature_importances_"):
                ...
                if len(fi) == len(selected_features):
                    for i, fname in enumerate(selected_features):
                        key = f"{name}__{fname}"
                        feature_importances[key] = float(fi[i])
    except Exception:
        pass
    ...
    # Guardar artefactos: ensemble + scaler + metadatos
    joblib.dump({
        "model": best_ensemble,
        "scaler": scaler,
...
def predict(paciente_id: int) -> dict:
    ...
    # Filtrar solo las features que el modelo espera (de la selección)
